...
import { AlertCircle, RefreshCw, GitPullRequest, Calendar, User } from "lucide-react"
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select a repository to view pull requests</CardDescription>
        </CardHeader>
      </Card>
    )
  }
    ...
    return (
      <Card>
        ...
        <CardContent>
          ...
          <Button variant="outline" size="sm" onClick={fetchPRs} className="mt-4">
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle>Pull Requests</CardTitle>
          <CardDescription>
            {pullRequests.length} pull request{pullRequests.length !== 1 ? "s" : ""} found
          </CardDescription>
        </div>
      ...
      <CardContent className="space-y-4">
        {pullRequests.length === 0 ? (
          <Alert>
            <AlertDescription>No pull requests found in this repository.</AlertDescription>
          </Alert>
        ) : (
          pullRequests.map((pr) => (
            <Card key={pr.pullRequestId} className="overflow-hidden">
              <div className="p-4">
                <div className="flex items-start justify-between">
                  ...
                  <Badge
                    variant={pr.status === "active" ? "default" : pr.status === "completed" ? "success" : "secondary"}
                  >
                    {pr.status}
                  </Badge>
                </div>
